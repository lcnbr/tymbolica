...
#let integration-model = atom-model(integrand, x)
#assert.eq(type(integration-model), bytes)
#let integral = integrate(integrand, x)
#let integration = integrate-with-steps(integrand, x)
...
Integral: #to-typst(integral); #integration.steps.len() nested Rubi steps
